...
import { loadTopViews, type TopRecord } from '../data/counts';
...
onMounted(async () => {
    list.value = await loadTopViews(TOP_N);
    loading.value = false;
});
...
const summary = computed(() => {
    ...
    return `Top1「${top.label}」，浏览 ${top.views ?? '—'}`;
});
</script>
    ...
    <div class="panel">
        ...
        <template v-else-if="!hasData">
            <div class="panel-title">
                浏览最多
                <span class="panel-note">暂无浏览数据</span>
            </div>
            <p class="empty">暂无浏览记录</p>
        </template>
        <n-collapse v-else :default-expanded-names="[]">
            <n-collapse-item name="views">
                <template #header>
                    <span class="panel-title">浏览最多</span>
                    <span class="panel-note">共 {{ list.length }} 条，{{ summary }}</span>
                </template>

                <div class="records-panel-body">
                    <div v-for="(item, i) in list" :key="'v-' + i" class="memory">
                        <n-tag
                            class="memory-label"
                            size="small"
                            :bordered="false"
...
.panel {
    margin-bottom: 20px;
    padding: 8px 16px;
    background: var(--bg-tint-yellow);            /* 浅金底：与👁️浏览/流量语义一致 */
    border-left: 3px solid #faad14; /* 金黄：流量/曝光主色 */
    border-radius: 2px;
}
